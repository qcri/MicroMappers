 ...
 */
public class SnopesFactFetch implements Tasklet {

    private static Logger logger = Logger.getLogger(SnopesFactFetch.class);
    ...
    public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
        ...
        if(!Util.isTimeToSnopesFetchRun()){
            logger.info("SnopesFactFetch need to wait more");
        ...
        try {
            ...
            String searchUrl = configProperties.getProperty(MicromappersConfigurationProperty.SNOPES_COM_FACT_CHECK_URL);
            HtmlPage page = client.getPage(searchUrl);

            List<HtmlElement> items = (List<HtmlElement>) page.getByXPath(".//ul[@class='post-list']/li[@class='item']") ;
            if(items.isEmpty()){
