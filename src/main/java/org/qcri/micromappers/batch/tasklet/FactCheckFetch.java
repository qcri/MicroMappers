 ...
 */
public class FactCheckFetch implements Tasklet {

    private static Logger logger = Logger.getLogger(FactCheckFetch.class);
    ...
    public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
        ...
        if(!Util.isTimeToFactCheckFetchRun()){
            logger.info("need to wait more for FactCheckFetch");
            return RepeatStatus.FINISHED;
        }
        else{
            logger.info("running for FactCheckFetch");
            Util.timeOfLastFactCheckProcessingMillis = System.currentTimeMillis();
        }

        URL url = new URL(configProperties.getProperty(MicromappersConfigurationProperty.FACT_CHECK_RSS_URL));
        HttpURLConnection httpcon = (HttpURLConnection)url.openConnection();
        // Reading the feed
        SyndFeedInput input = new SyndFeedInput();
        SyndFeed feed = input.build(new XmlReader(httpcon));
        List entries = feed.getEntries();
        String author = "factcheck.org";
        entries.forEach(item -> {
          //  System.out.println(item);
            ...
            if(definition == null){
                ...
                globalEventDefinition.setDescription(((SyndEntryImpl) item).getDescription().getValue());
